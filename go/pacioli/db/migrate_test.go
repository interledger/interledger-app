...
// copies id into reference_id for rows created before the column existed.
func TestBackfillTransferReferenceIDs(t *testing.T) {
	...
	// reference_id is left NULL, as it is for rows written before the backfill runs.
	...
	require.NoError(t, backfillTransferReferenceIDs(ctx, connString))

	var txnID string
	require.NoError(t, conn.GetContext(ctx, &txnID,
		"SELECT reference_id FROM ledger_transfers WHERE id=$1", transferID))
