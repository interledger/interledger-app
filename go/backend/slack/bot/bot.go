...
func NewSlackCommandHandler(b Backends) http.HandlerFunc {
	signingSecret := "af9e155e443e41b878753c44e8c0d9c8" // TODO: Replace with env var. This is BarnardTest bot.
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		switch s.Command {
		case "/fynpay": // TODO: pick a better name or a name per env
			team := s.TeamID
			...
			if errors.Is(err, slack.ErrNotFound) {
				data, err := json.Marshal(&ext_slack.Msg{Text: "You do not have a fynbos wallet, please create one on https://fynbos.app"})
				if err != nil {
					w.WriteHeader(http.StatusInternalServerError)
					return
				}
			...
			if errors.Is(err, slack.ErrNotFound) {
				data, err := json.Marshal(&ext_slack.Msg{Text: fmt.Sprintf("@%s do not have a fynbos wallet, please have them create one on https://fynbos.app", normalizeUserID(receiverSlackID))})
				if err != nil {
					w.WriteHeader(http.StatusInternalServerError)
					return
				}
			...
			if errors.Is(err, linkedaccounts.ErrNotFound) {
				data, err := json.Marshal(&ext_slack.Msg{Text: fmt.Sprintf("@%s is not capable of receiveing payments, please have them add an account that can receive payments", extractUserName(receiverSlackID))})
				if err != nil {
					w.WriteHeader(http.StatusInternalServerError)
					return
				}
			...
			data, err := json.Marshal(&ext_slack.Msg{Text: fmt.Sprintf("Your payment to @%s requires your authorization %s", extractUserName(receiverSlackID), authURL)})
			if err != nil {
				w.WriteHeader(http.StatusInternalServerError)
				return
			}
...
func extractUserName(userID string) string {
	idx := strings.Index(userID, "|") + 1
	return userID[idx : len(userID)-1]
}
