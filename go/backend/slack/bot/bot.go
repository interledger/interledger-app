...
func NewSlackCommandHandler(b Backends) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case "/fynpay": // TODO: pick a better name or a name per env
			...
			if errors.Is(err, linkedaccounts.ErrNotFound) {
				data, err := json.Marshal(&ext_slack.Msg{Text: fmt.Sprintf("@%s is not capable of sending, please have them add an account that can receive payments", normalizeUserID(receiverSlackID))})
				if err != nil {
					w.WriteHeader(http.StatusInternalServerError)
					return
				}
			...
			data, err := json.Marshal(&ext_slack.Msg{Text: fmt.Sprintf("Your payment to %s requires your authorization %s", normalizeUserID(receiverSlackID), authURL)})
			if err != nil {
				w.WriteHeader(http.StatusInternalServerError)
				return
			}
