package jobs

import (
	"context"
	"time"

	"gitlab.com/fynbos/backend/currency"
	"gitlab.com/fynbos/backend/providers/pti"
	pti_ops "gitlab.com/fynbos/backend/providers/pti/ops"
	"go.temporal.io/api/enums/v1"
	"go.temporal.io/sdk/temporal"
	"go.temporal.io/sdk/workflow"
)

func RerunCreatePTIWallets(ctx workflow.Context) error {
	var a *Activity
	ao := workflow.ActivityOptions{
		StartToCloseTimeout: 10 * time.Minute,
		RetryPolicy: &temporal.RetryPolicy{
			MaximumAttempts: 3,
		},
	}
	ctx = workflow.WithActivityOptions(ctx, ao)

	var walletIDs []string
	err := workflow.ExecuteActivity(ctx, a.ListWalletsWithPTIUsers).Get(ctx, &walletIDs)
	if err != nil {
		return err
	}

	for _, w := range walletIDs {
		childCtx := workflow.WithChildOptions(ctx, workflow.ChildWorkflowOptions{
			WorkflowID:            "pti_create_wallet_" + w + "_" + currency.USD.String(),
			ParentClosePolicy:     enums.PARENT_CLOSE_POLICY_ABANDON,
			WorkflowIDReusePolicy: enums.WORKFLOW_ID_REUSE_POLICY_TERMINATE_IF_RUNNING,
		})
		err = workflow.ExecuteChildWorkflow(childCtx, pti_ops.CreateWalletWorkflow, pti.CreateWalletArgs{
			WalletID: w,
			Currency: currency.USD,
			Nickname: "USD Balance",
			Title:    "USD Balance",
		}).GetChildWorkflowExecution().Get(ctx, nil)
		if err != nil {
			return err
		}
	}

	return nil
}

func (a *Activity) ListWalletsWithPTIUsers(ctx context.Context) ([]string, error) {
	var walletIDs []string
	err := a.b.DB().SelectContext(ctx, &walletIDs, "SELECT wallet_id FROM pti_users;")
	if err != nil {
		return nil, err
	}

	return walletIDs, nil
}
