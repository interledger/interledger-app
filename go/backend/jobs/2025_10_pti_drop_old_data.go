...
func (a *Activity) PtiDropWalletOldDataActivity(ctx context.Context) (err error) {
	...
	for _, wla := range wlas {
		...
		qTransactions := `delete from transactions tons
						  using transfers ters
						  where tons.id = ters.transaction_id
						    and tons.asset_code = 'USD'
						    and ters.asset_code = 'USD'
						    and tons.provider = 'pti'
						    and ters.linked_acc_id = $1;`
		...
		// wallet_kyc_status
		if _, err := tx.Exec("update wallet_kyc_status set status = 0 where wallet_id = $1;", wla.WalletId); err != nil {
	...
	// if _, err = tx.Exec("truncate table pti_transactions;"); err != nil {
	// 	logger.Error("pti_transactions table", err)
	// 	goto rollback
	// }

	// pti_users table
	if _, err = tx.Exec("truncate table pti_users;"); err != nil {
		logger.Error("pti_users table", err)
		goto rollback
	}
...
rollback:
	if rbErr := tx.Rollback(); rbErr != nil {
		logger.Error("failed to rollback", rbErr) // ouch! the worst thing can happen
		return rbErr
	}
	return err
}
