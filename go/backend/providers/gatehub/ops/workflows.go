...
func ProcessGatehubWithdrawal(ctx workflow.Context, walletID, transactionID string) error {
	...
	err := workflow.ExecuteActivity(ctx, a.ReserveBalance, transactionID, walletID).Get(ctx, nil)
	if err != nil {
	...
	err = workflow.ExecuteActivity(ctx, a.FinalizeBalance, transactionID, walletID).Get(ctx, nil)
	if err != nil {
		handleFailedWithdrawal(ctx, a, walletID, transactionID)
		return err
	}
