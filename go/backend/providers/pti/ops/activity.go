...
func (a *Activity) CheckUserAssessmentAccepted(ctx context.Context, walletID string) error {
	...
	if err != nil {
		log.Error("pti activity (checkUserAssessmentAccepted): failed to fetch external user")
		return nil
	}

	dob, err := time.Parse(time.DateOnly, externalUser.DateOfBirth)
	if err != nil {
		log.Error("pti activity (checkUserAssessmentAccepted): failed to parse date of birth")
		return nil
	}
	...
	if len(externalUser.Addresses) > 0 {
		args.Address.State = externalUser.Addresses[0].StateCode.Code
		args.Address.Line1 = externalUser.Addresses[0].Street
		args.Address.City = externalUser.Addresses[0].City
		args.Address.ZipCode = externalUser.Addresses[0].PostalCode
	}

	_, err = a.b.KYC().UpdateIndividualDetails(ctx, args)
	if err != nil {
		log.Error("pti activity (checkUserAssessmentAccepted): failed to update individual details")
	}
