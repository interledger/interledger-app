...
func HandleAssessmentUpdate(ctx context.Context, b Backends, data []byte) error {
	...
	default:
		...
		slack.SendToChannel(ctx, slack.ChannelSignupKYC, "wallet-info-bot", fmt.Sprintf("fiant webhook: kyc assessment status=%s walletID=%s", assessmentData.Assessment, ptiUser.WalletID))
	}
...
func HandleTransactionStatus(ctx context.Context, b Backends, raw json.RawMessage, w http.ResponseWriter) error {
	...
	default:
		...
		slack.SendToChannel(ctx, slack.ChannelTransaction, "wallet-info-bot", fmt.Sprintf("fiant webhook: transaction status=%s walletID=%s", payload.Status, payload.UserID))
	}
