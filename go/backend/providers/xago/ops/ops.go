...
func CreateBeneficiary(ctx context.Context, b Backends, walletID string) (xago.Await, error) {
	wo := client.StartWorkflowOptions{
		ID:                       "xago_create_beneficiary_" + walletID,
		TaskQueue:                "backend",
		WorkflowExecutionTimeout: 2 * time.Minute,
		WorkflowIDReusePolicy:    enums.WORKFLOW_ID_REUSE_POLICY_TERMINATE_IF_RUNNING,
	}
	...
	if workflowStatus == enums.WORKFLOW_EXECUTION_STATUS_RUNNING {
		...
		await, executeErr = b.Temporal().ExecuteWorkflow(ctx, wo, CreateBeneficiaryWorkflow, walletID)
	}
	if executeErr != nil {
		return nil, fmt.Errorf("%w %s", xago.ErrInternal, err)
	}
