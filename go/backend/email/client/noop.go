...
func (n *noopClient) SendMigrationEmail(_ context.Context, subject, sendTo, firstName string, paragraphs []map[string]interface{}, bcc string) error {
	log.Info("NOT SENDING: migration email", zap.String("email", sendTo), zap.String("subject", subject), zap.String("firstName", firstName), zap.Int("paragraphs", len(paragraphs)), zap.String("bcc", bcc))
	return nil
}
