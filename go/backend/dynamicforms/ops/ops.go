...
func ExportFormResults(ctx context.Context, b Backends, formID string, writer io.Writer) error {
	...
	err := b.DB().SelectContext(ctx, &formDatas, "SELECT data FROM dynamic_forms WHERE form_id = $1", formID)
	if errors.Is(err, sql.ErrNoRows) {
	...
	for _, item := range formDatas {
		formData, err := strconv.Unquote(item)
		if err != nil {
			return fmt.Errorf("%w %s %s", dynamicforms.ErrInternal, err, item)
		}

		resultSlice = append(resultSlice, formData)
	}

	result := "[" + strings.Join(resultSlice, ",") + "]"

	err = jsonToCSV(strings.NewReader(result), writer)
	if err != nil {
		return fmt.Errorf("%w %s", dynamicforms.ErrInternal, err)
	}
