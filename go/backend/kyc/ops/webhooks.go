...
func notifyPersonaReview(ctx context.Context, js json.RawMessage) {
	...
	slack.SendToChannel(ctx, "wallet-info-bot", fmt.Sprintf("New Persona review in [%s] link [https://app.withpersona.com/dashboard/inquiries/%s]",
...
func accountTagAddedWebhook(ctx context.Context, b Backends, pc persona.Client, js json.RawMessage) error {
	...
	// Too many state tags.
	if stateTags > 1 {
		slack.SendToChannel(ctx, "wallet-info-bot", fmt.Sprintf("Persona account in [%s] is in unknown state. link [https://app.withpersona.com/dashboard/accounts/%s]",
