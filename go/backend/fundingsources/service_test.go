...
func TestSetMxFundingSourceMask(t *testing.T) {
	...
	err = fs.SetMxFundingSourceMask(ctx, mxFs.ID)
	assert.NoError(t, err)

	fundingsource, err := fs.Get(ctx, mxFs.ID)
	assert.NoError(t, err)
	assert.Equal(t, "6789", fundingsource.Mask)
}
