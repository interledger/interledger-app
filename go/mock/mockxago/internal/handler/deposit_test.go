...
// NewProcessDepositHandler Tests
...
func TestNewProcessDepositHandler_ReturnsFunctionHandler(t *testing.T) {
	...
	// Verify the handler is a JobHandler (function type)
	// The function type will be jobs.JobHandler
...
func TestNewProcessDepositHandler_ProcessesDepositJob(t *testing.T) {
	...
	// Create a job with deposit data
	job := &models.Job{
		ID:      "job_123",
		JobType: "process_deposit",
		Status:  "pending",
		Data: map[string]interface{}{
			"transaction_id":    "dep_test_123",
			"account_id":        "acc_process",
			"amount":            5000.00,
			"currency":          "ZAR",
			"deposit_reference": "ref_123",
		},
	}

	// Get the handler and execute the job
	handler := h.NewProcessDepositHandler()
	err = handler(context.Background(), job)
	require.NoError(t, err)

	// Verify balance was credited
	available, _, err := store.GetBalance(context.Background(), "wallet_deposit", "ZAR")
	require.NoError(t, err)
	assert.Equal(t, 5000.00, available)

	// Verify deposit status was updated
	updatedDeposit, err := store.GetDeposit(context.Background(), "dep_test_123")
	require.NoError(t, err)
	assert.Equal(t, "completed", updatedDeposit.Status)
}
...
func TestNewProcessDepositHandler_WithMultipleDeposits(t *testing.T) {
	...
	for _, d := range deposits {
		...
		err := handler(context.Background(), job)
		require.NoError(t, err)
		expectedTotal += d.amount
	}

	// Verify total balance
	available, _, err := store.GetBalance(context.Background(), "wallet_multi", "ZAR")
	require.NoError(t, err)
	assert.Equal(t, expectedTotal, available)
}
...
func TestSimulateTestDeposit_MultipleDeposits(t *testing.T) {
	...
	// Make multiple deposits to same account
	for i := 1; i <= 3; i++ {
		body := map[string]interface{}{
			"accountId":    "multi_acc",
...
func TestSimulateTestDeposit_InvalidAmount(t *testing.T) {
	...
	// Create test account
	subAccount := &models.SubAccount{
		AccountID: "invalid_amt",
		WalletID:  "wallet_invalid",
	}
	err := store.SaveSubAccount(context.Background(), subAccount)
	require.NoError(t, err)
	...
	tests := []struct {
		...
		{"negative amount", -100.00, true},
		{"zero amount", 0, true},
		{"normal amount", 1000.00, false},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantError {
				assert.True(t, w.Code >= 400, "expected error response for %s", tt.name)
			} else {
				assert.Equal(t, http.StatusOK, w.Code)
			}
		})
...
func TestListCompanyDeposits_Success(t *testing.T) {
	...
	subAccount := &models.SubAccount{
		AccountID: "list_test_acc",
		WalletID:  "wallet_list",
	}
	err := store.SaveSubAccount(context.Background(), subAccount)
	require.NoError(t, err)

	// Add a couple of deposits
	for i := 1; i <= 2; i++ {
		deposit := &models.Deposit{
			ID:        "dep_list_" + strconv.Itoa(i),
			AccountID: "list_test_acc",
			Amount:    float64(i * 1000),
			Currency:  "ZAR",
			Status:    "completed",
