...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  if (
    ...
    paymentDetails.incomingAmount === undefined
  ) {
    throw data(
      {
        code: 'QUICKPAY_SESSION_ERROR',
  ...
  const paymentData = {
    ...
    receiverName: receiverWalletAddress?.publicName
      ? receiverWalletAddress.publicName
      : '',
...
export default function Page() {
  const { assetCode, isQuote, quoteData, paymentData } =
  ...
  return (
    <WalletGrid>
      <GridColumn className='col-span-full mx-auto mt-20'>
        <AmountDisplay
          displayAmount={String(paymentData.amount.amount)}
          assetCode={assetCode}
        />
...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (intent === 'confirm') {
    if (
      sessionData.quote === undefined ||
      sessionData.senderAddress === undefined
    ) {
      throw data(
        {
          code: 'QUICKPAY_SESSION_ERROR',
