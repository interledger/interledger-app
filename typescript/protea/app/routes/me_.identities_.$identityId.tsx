...
import {AnchorRouter, Button, ButtonRouter, Card, FynbosIcon, Layouts, Router} from '~/components'
import {getPublicIdentity, getPublicWalletDetails, getWalletPaymentPointer} from '~/lib/wallet.server'
      ...
      {identity.state == 'verified' && (
        <Card>
          ...
          <p className="mt-4">
            ...
            <AnchorRouter to={`https://twitter.com/${identity.identifier}`} className="text-primary"> @{identity.identifier} </AnchorRouter>
            is the same person as
            <AnchorRouter to={identity.wallet} className="text-primary"> {removeProtocol(identity.wallet)} </AnchorRouter>
      ...
      {!isUser && (
        <Card className='space-y-4'>
          ...
          <p className='text-sm text-medium'>
            Fynbos is a digital wallet for verifying identities, paying contacts, and building trust.
          </p>
          <Router
