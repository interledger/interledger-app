...
export function Amount() {
  ...
  const _onChangeAmount = useCallback<ChangeEventHandler<HTMLInputElement>>(
    ...
    [account?.id, address?.walletUrl, quoteFetcher, note, setAmount]
  )

  const _onChangeLinkedAccount = useCallback(
    (event: SelectOptions) => {
      console.log('Changed card', event)
    ...
    [address?.walletUrl, amount, note, quoteFetcher, setAccount]
  )
  ...
  const _onChangeNote = useCallback<ChangeEventHandler<HTMLInputElement>>(
    ...
    [account?.id, address?.walletUrl, amount, quoteFetcher, setNote]
  )

  useEffect(() => {
    console.log('quoteFetcher', quoteFetcher.data)
    if (quoteFetcher.data?.quoteId) {
      setQuoteId(quoteFetcher.data?.quoteId)
    }
    if (quoteFetcher.data?.type) {
  ...
  }, [account?.id, address?.walletUrl, amount, note, quoteFetcher])
