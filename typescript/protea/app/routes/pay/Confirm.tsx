...
export function Confirm() {
  ...
  const [address, account, displayAmount, note, publicWalletInfo, setStep] =
    usePayStore((state) => [
      state.address,
      state.account,
      state.displayAmount,
      ...
      <Card>
        <CardContent>
          <Checkbox
            ...
            className='flex'
            aria-invalid={
              Boolean(fetcher.data?.errors?.serviceAgreement) || undefined
            }
            aria-describedby={
              fetcher.data?.errors?.serviceAgreement
                ? 'serviceAgreement-error'
                : undefined
            }
            errorMessage={fetcher.data?.errors?.serviceAgreement}
          >
