...
import { Alert, AlertContent, Card, CardButton, CardContent, CardHeader, CardLink, CardTitle, Chip, ChipColor, Dialog, DiscordIcon, FynbosIcon, Icon, Layouts, LinkedInIcon, Router, SlackIcon, TextButton, TwitterIcon } from '~/components';
...
export default function Page() {
  const { formattedAmount, formattedTime, formattedDate, mask, note, publicWalletInfo } = useLoaderData<typeof loader>()
  const [showDialog, setShowDialog] = useState<boolean>(false)

  return <>
    <Alert color={ChipColor.green}>
      <AlertContent>
        You have collected a payment of {formattedAmount} to your card ending {mask}.
      </AlertContent>
    </Alert>
    <Card>
      <CardContent>
    ...
    <Card>
      ...
      <CardContent className='flex space-x-4'>
        <div className='w-16 h-16 rounded-full bg-nav flex items-center justify-center'><FynbosIcon /></div>
        <div className='flex-1 flex-col space-y-4'>
          <p className='text-medium'>
            Sign up to Fynbos and get rewarded for sending and receiving payments.
          </p>
          <Router
