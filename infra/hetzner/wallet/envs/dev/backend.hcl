job "backend" {
  ...
  group "backend" {
    ...
    service {
      ...
      tags = [
        "traefik.enable=true",
        "traefik.http.routers.backend.rule=Host(`eu1.fynbos.dev`) && PathPrefix(`/webhooks`)",
        "traefik.http.routers.backend.rule=Host(`eu1.fynbos.me`)",
      ]
    }
    ...
    task "backend-migrations" {
      ...
      template {
        data = <<EOH
          DB_URL_WITH_CERTS="postgres://{{with secret "database-dev/creds/backend"}}{{.Data.data.username}}:{{.Data.password}}{{end}}@localhost:5432/backend?sslmode=disable"
          PACIOLI_DB_URL_WITH_CERTS="postgres://{{with secret "database-dev/creds/pacioli"}}{{.Data.data.username}}:{{.Data.password}}{{end}}@localhost:5432/pacioli?sslmode=disable"
    ...
    task "backend" {
      ...
      template {
        data = <<EOH
          FYNBOS_ENV=dev
          {{with secret "database-dev/creds/backend"}}
          ...
          {{with secret "database-dev/creds/pacioli"}}
          ...
          VAULT_AUTH_PATH: "k8s-dev-use2"
          VAULT_TRANSIT_ENGINE_PATH: "transit/k8s-dev-use2/backend/backend"
