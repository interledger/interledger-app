job "temporal" {
  ...
  group "temporal" {
    ...
    service {
      ...
      tags = [
        "traefik.enable=true",
        "traefik.http.routers.temporalui.rule=Host(`temporal-dev.mgnt.fynbos.dev`)"
      ]
    }
    ...
    task "auto-setup" {
      ...
      config {
        image = "localhost:5002/auto-setup:1.18.5"
      ...
      template {
        ...
        DB_PORT=5432
        {{with secret "database-prod/creds/temporal"}}
        POSTGRES_USER={{.Data.username}}
        POSTGRES_PWD={{.Data.password}}
        {{end}}
        {{with secret "database-prod/creds/temporal_visibility"}}
