job "kratos" {
  ...
  group "kratos" {
    ...
    task "kratos-migrate" {
      ...
      template {
        data = <<EOH
          DSN={{with secret "database-prod/creds/kratos"}}"postgres://{{.Data.username}}:{{.Data.password}}@localhost:5432/kratos?sslmode=disable"{{end}}
        EOH
    ...
    task "kratos" {
      ...
      template {
        data = <<EOH
          DSN={{with secret "database-prod/creds/kratos"}}"postgres://{{.Data.username}}:{{.Data.password}}@localhost:5432/kratos?sslmode=disable"{{end}}
          {{- with secret "kv/data/prod/kratos/config" -}}
