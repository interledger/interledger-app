  ...
  group "backend" {
    ...
    service {
      ...
      tags = [
        ...
        "traefik.http.routers.rafiki-connector.rule=Host(`fynbos.me`) && Path(`/ilp`)",
    ...
    service {
      ...
      tags = [
        "traefik.enable=true",
        "traefik.http.routers.rafiki-openpayments.rule=Host(`fynbos.me`) && PathPrefix(`/incoming-payments`)",
        "traefik.http.routers.rafiki-openpayments.rule=Host(`fynbos.me`) && PathPrefix(`/outgoing-payments`)",
        "traefik.http.routers.rafiki-openpayments.rule=Host(`fynbos.me`) && PathPrefix(`/quotes`)",
        "traefik.http.routers.rafiki-openpayments.rule=Host(`fynbos.me`) && PathPrefix(`/.well-known`)"
      ]
    }
    ...
    service {
      ...
      tags = [
        ...
        "traefik.http.routers.rafiki-auth.rule=Host(`fynbos.me`) && Path(`/gnap`)",
        "traefik.http.routers.rafiki-auth.rule=Host(`fynbos.me`) && PathPrefix(`/token`)",
        "traefik.http.routers.rafiki-auth.rule=Host(`fynbos.me`) && PathPrefix(`/interact`)",
        "traefik.http.routers.rafiki-auth.rule=Host(`fynbos.me`) && PathPrefix(`/continue`)",
    ...
    service {
      ...
      tags = [
        "traefik.enable=true",
        "traefik.http.routers.rafiki-frontend.rule=Host(`rafiki.mgnt.fynbos.dev`)"
      ]
    }
    ...
    task "auth" {
      ...
      template {
        data = <<EOF
        AUTH_DATABASE_URL="postgres://{{with secret "database-prod/creds/rafiki_auth"}}{{.Data.data.username}}:{{.Data.password}}{{end}}@localhost:5432/rafiki_auth?sslmode=disable"
        {{with secret "kv/data/prod/rafiki_auth/config"}}
    ...
    task "backend" {
      ...
      template {
        data = <<EOF
        DATABASE_URL="postgres://{{with secret "database-prod/creds/rafiki_backend"}}{{.Data.data.username}}:{{.Data.password}}{{end}}@localhost:5432/rafiki_auth?sslmode=disable"
        {{with secret "kv/data/prod/rafiki_backend/config"}}
