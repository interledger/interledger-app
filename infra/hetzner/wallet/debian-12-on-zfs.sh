...
zfs create -o com.sun:auto-snapshot=false  "$v_rpool_name/var/lib/docker"
...
chroot_execute "DEBIAN_FRONTEND=noninteractive apt install dosfstools"
chroot_execute "mkdosfs -F 32 -s 1 -n EFI ${v_selected_disks[0]}-part1"
...
chroot_execute "grub-install --target=x86_64-efi --efi-directory=/boot/efi --bootloader-id=debian --recheck --no-floppy"

echo "======= mirror grub =========="
chroot_execute "umount /boot/efi"
for ((i = 1; i < ${#v_selected_disks[@]}; i++)); do
  dd if="${v_selected_disks[0]}-part1" of="${v_selected_disks[i]}-part1"
  chroot_execute "efibootmgr -c -g -d ${v_selected_disks[i]} -p 1 -L \"debian-${i}\" -l '\EFI\debian\grubx64.efi'"
done
...
# Not sure here. We've already mounted the EFI partition by uuid when setting up GRUB
