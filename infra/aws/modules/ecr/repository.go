...
func NewPrivateRepository(ctx *pulumi.Context, name string, accountID string, pullAccountIds []string) (*ecr.Repository, error) {
	...
	accessPolicy, err := AllowCrossAccountPullAccessToRepo(ctx, pullAccountIds)
	if err != nil {
		return nil, err
	}
	...
	repo, err := ecr.NewRepository(ctx, name, &ecr.RepositoryArgs{
		...
		RepositoryPolicyText: pulumi.String(accessPolicy.Json),
...
func AllowCrossAccountPullAccessToRepo(ctx *pulumi.Context, accountIds []string) (*iam.GetPolicyDocumentResult, error) {

	var identifiers []string
	for _, id := range accountIds {
		identifiers = append(identifiers, fmt.Sprintf("arn:aws:iam::%s:root", id))
	}

	policy, err := iam.GetPolicyDocument(ctx, &iam.GetPolicyDocumentArgs{
		Statements: []iam.GetPolicyDocumentStatement{
			{
				Sid:    utils.StringPtr("AllowPull"),
