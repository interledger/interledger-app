...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		// Create required service account roles first for ebs encryption. We should really move the permission out of
		// the key creation
		_, err = iam.NewServiceLinkedRole(ctx, "autoscaling-role", &iam.ServiceLinkedRoleArgs{
		...
		_, err = secure_baseline.NewDevCrossAccountIamRole(ctx, securityAccountId)
		if err != nil {
			return err
		}
