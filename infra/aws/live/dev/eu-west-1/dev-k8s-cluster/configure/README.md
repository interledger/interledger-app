...
```sh
aws-vault exec <dev-account> -- pulumi stack output -C ../provision kubeconfig | jq > kubeconfig.yaml
export KUBECONFIG=./kubeconfig.yaml
```
Next, we need to look up the clusters oidc from the provision stack and set it in this projects config.
```sh
aws-vault exec <dev-account> -- pulumi stack output -C ../provision oidcProvider
