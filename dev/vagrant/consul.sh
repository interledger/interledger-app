...
sudo DEBIAN_FRONTEND=noninteractive apt-get --assume-yes install curl unzip jq wget < /dev/null > /dev/null
mkdir -p /etc/consul
...
EOF
  ...
  # if consul is not installed, download and install
    ...
    LATEST_URL=$(curl -sL https://releases.hashicorp.com/consul/index.json | jq -r '.versions[].builds[].url' | sort -t. -k 1,1n -k 2,2n -k 3,3n -k 4,4n | egrep -v 'rc|ent|beta' | egrep "1.17.3_linux.*$ARCH" | sort -V | tail -1)
