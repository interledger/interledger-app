...
Feature: GateHub card transaction processing
  ...
  Scenario: Purchase reversal refunds and creates a pending deposit with Refund note
    When I trigger a card purchase transaction for 'card-tx-user'
    ...
    And a pending deposit transaction should exist in the database for 'card-tx-user' with note "Refund"
