...
func writeScenarioODS(dbPath string, scenario *godog.Scenario) error {
	...
	if err != nil {
		return fmt.Errorf("list accounts for ods: %w", err)
	}
	lines, err := store.GetAllLines()
	if err != nil {
		return fmt.Errorf("list journal lines for ods: %w", err)
	}

	path := scenarioODSPath(scenario)
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
		return fmt.Errorf("create ods output dir: %w", err)
	}
	if err := writeODS(path, scenario, accounts, lines); err != nil {
...
func scenarioODSPath(scenario *godog.Scenario) string {
	...
	return filepath.Join("..", "output", slug(group)+"__"+slug(name)+".ods")
}

func writeODS(path string, scenario *godog.Scenario, accounts []engine.Account, lines []engine.JournalLine) error {
	...
	files := map[string]string{
		...
		"content.xml":           contentXML(scenario, accounts, lines),
...
func contentXML(scenario *godog.Scenario, accounts []engine.Account, lines []engine.JournalLine) string {
