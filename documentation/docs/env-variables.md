...
| `XAGO_TRAVEL_RULE_EMAIL` | Email address to send the Travel Rule CSV report to (every 24 hours) | Yes | Not set locally |
| `XAGO_TRAVEL_RULE_PGP_PUBLIC_KEY` | PGP public key (armored text) used to encrypt the Travel Rule CSV before emailing. When absent the Travel Rule email workflow is skipped entirely. | Yes | Not set locally |
